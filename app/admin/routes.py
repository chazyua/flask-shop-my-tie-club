...
def list_categories():
	...
	return render_template('admin/categories/categories.html', title = "Product Categories", categories = categories)
...
def add_category():
	...
	if form.validate_on_submit():
		# random_hex = secrets.token_hex(8) ##not deleting file with category, get back to it later
		filename = request.files['image']
		_, f_ext = os.path.splitext(filename.filename)
		name = form.name.data
		picture_fn = name.lower().strip() + f_ext
...
def edit_category(id):
	...
	if form.validate_on_submit():
		...
		_, f_ext = os.path.splitext(filename.filename)
		name = form.name.data
		picture_fn = name.lower().replace(' ', '') + f_ext
...
def delete_product(id):
	...
	previous_img_name = product.product_name.lower().replace(' ', '') + f_ext
	img_dir = Config.UPLOADED_PHOTOS_DEST+'/'
	#remove the changed picture from the folder
	os.remove(img_dir+previous_img_name)		
